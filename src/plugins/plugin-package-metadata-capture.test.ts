...
describe("streaming plugin source capture (issue #155728)", () => {
  ...
  it("writes each chunk completely after short writes", () => {
    ...
    vi.spyOn(fs, "writeSync").mockImplementation(writeShortChunk as typeof fs.writeSync);
  ...
  it.each(["zero progress", "ENOSPC"])(
    ...
    (failure) => {
      ...
      const realWriteSync = fs.writeSync;
      const writePrefix = (fd: number, buffer: NodeJS.ArrayBufferView, offset = 0) =>
        realWriteSync(fd, buffer, offset, 17);
