...
git rev-parse HEAD
git diff --exit-code 9ba1f0bab890b871710c0b025775b9a1f106cb47 HEAD -- . ':(exclude).github/workflows/codex-stream-memory-validation.yml' ':(exclude).github/diagnostics/stream-validation.sh' ':(exclude).github/diagnostics/check-short-write-red.py' ':(exclude).github/diagnostics/record-tested-tree.py'
node --version
pnpm --version
