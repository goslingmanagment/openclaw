...
git rev-parse HEAD
git diff --exit-code c76b73bb3510eb4238fbce080558ff007153abae HEAD -- . ':(exclude).github/workflows/codex-stream-memory-validation.yml' ':(exclude).github/diagnostics/stream-validation.sh' ':(exclude).github/diagnostics/check-short-write-red.py' ':(exclude).github/diagnostics/record-tested-tree.py'
node --version
pnpm --version
